...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($transaksi_id) {
            $query = "UPDATE transaksi SET tanggal = ?, kategori_id = ?, jumlah = ?, deskripsi = ?, jenis_transaksi = ? WHERE transaksi_id = ? AND user_id = ?";
            $stmt = mysqli_prepare($conn, $query);
            ...
            $query = "INSERT INTO transaksi (tanggal, kategori_id, jumlah, deskripsi, jenis_transaksi, user_id) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $query);
...
$query = "SELECT t.*, k.nama_kategori FROM transaksi t LEFT JOIN kategori k ON t.kategori_id = k.kategori_id WHERE $where_clause ORDER BY t.tanggal DESC LIMIT ?, ?";
$params[] = $start;
$params[] = $per_page;
$param_types .= "ii";
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $base_url; ?>/css/secret-role.css">
    <script src="https://cdn.jsdelivr.net/particles.js/2.0.0/particles.min.js"></script>
    <script src="<?php echo $base_url; ?>/js/secret-role.js"></script>
    <?php endif; ?>
</head>
...
<body class="<?php echo $is_secret_role ? 'secret-role-body' : ''; ?>">
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <h1 class="<?php echo $is_secret_role ? 'secret-role' : ''; ?>">Transaksi Keuangan</h1>
            <button class="btn <?php echo $is_secret_role ? 'secret-role-btn glow-on-hover' : ''; ?>"
                onclick="openModal('tambah')"><i class="fas fa-plus"></i> Tambah Transaksi</button>
        </div>
        ...
        <div class="filter-container">
            <form method="GET" class="filter-form">
                ...
                <input type="date" name="tanggal_akhir" value="<?php echo htmlspecialchars($tanggal_akhir); ?>"
                    class="<?php echo $is_secret_role ? 'glow-on-hover' : ''; ?>">
                <select name="jenis" class="<?php echo $is_secret_role ? 'glow-on-hover' : ''; ?>">
                ...
                <button type="submit"
                    class="<?php echo $is_secret_role ? 'secret-role-btn glow-on-hover' : ''; ?>">Filter</button>
                <a href="<?php echo $base_url; ?>/dashboard/transaksi.php"
                    class="btn <?php echo $is_secret_role ? 'secret-role-btn' : ''; ?>">Reset</a>
            </form>
        </div>
                    ...
                    <tr class="<?php echo $is_secret_role ? 'glow-on-hover' : ''; ?>">
                        ...
                        <td>
                            <button onclick="editTransaksi(<?php echo $transaksi['transaksi_id']; ?>)"
                                class="btn-edit <?php echo $is_secret_role ? 'secret-role-btn' : ''; ?>"><i
                                    class="fas fa-edit"></i></button>
                            <button onclick="hapusTransaksi(<?php echo $transaksi['transaksi_id']; ?>)"
                                class="btn-delete <?php echo $is_secret_role ? 'secret-role-btn' : ''; ?>"><i
                ...
                <form id="transaksiForm" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="kategori_id" id="kategori_id" required
                            ...
                            <option value="<?php echo $kategori['kategori_id']; ?>"
                                ...
                                <?php echo htmlspecialchars($kategori['nama_kategori']); ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" id="deskripsi"
                            class="<?php echo $is_secret_role ? 'glow-on-hover' : ''; ?>"></textarea>
                    </div>
                    <button type="submit"
                        class="btn-submit <?php echo $is_secret_role ? 'secret-role-btn' : ''; ?>">Simpan</button>
                </form>
            </div>
        </div>
    </div>
    ...
    function editTransaksi(id) {
        openModal('edit');
        fetch(`${baseUrl}/config/get_transaksi.php?id=${id}`)
            .then(response => {
